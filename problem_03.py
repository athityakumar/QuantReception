...
class UserMainCode(object):
    ...
    def solution(cls, r, N):
        ...
        def compute_delta(spot):
            delta = [log(spot[i+1]) - log(spot[i]) for i in range(len(spot)-1)]
            return delta

        def compute_sigma(spot):
            delta = compute_delta(spot)
            sigma = np.std(delta)
            return sigma
        ...
        X0 = X_SPOT[-1]
        Y0 = Y_SPOT[-1]
        delta_x = compute_delta(X_SPOT)
        delta_y = compute_delta(Y_SPOT)
        corr = np.corrcoef(delta_x, delta_y)[0][-1]
        mu_x = mean(delta_x)
        mu_y = mean(delta_y)
        sigma_x = compute_sigma(X_SPOT)
        sigma_y = compute_sigma(Y_SPOT)

        mu = [mu_x, mu_y]
        cov = np.cov(delta_x, delta_y)
        #[
        #    [sigma_x*sigma_x, corr*sigma_x*sigma_y],
        #    [corr*sigma_x*sigma_y, sigma_y*sigma_y]
        #]

        n = 252
        m = 1000
        TN = 0.25 * N
        times = [0] + [(t+1)*TN/n for t in range(n)]
        ...
        for j in range(m):
            ...
            for i in range(n):
                epsilon_1, epsilon_2 = multivariate_normal(mu, cov)
                delta_w_x = epsilon_1 * sqrt(delta_t)
                delta_w_y = epsilon_2 * sqrt(delta_t)
                delta_x = r * prev_x * delta_t + sigma_x * delta_w_x
                delta_y = r * prev_y * delta_t + sigma_y * delta_w_y
                prev_x += delta_x
                prev_y += delta_y
            Xn = prev_x
            Yn = prev_y
            if Xn >= 0.75*X0 and Xn <= 1.25*X0 and Yn >= 0.75*Y0 and Yn <= 1.25*Y0:
                payment = 0.5*Xn/X0 + 0.5*Yn/Y0 - 1
                payment = payment if payment > 0 else 0
                payment_multiverse.append(payment)

        expected_payoff = mean(payment_multiverse)
        return expected_payoff*exp(-r*(TN+1))
